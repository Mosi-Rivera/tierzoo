...
const basic_attack = (self,enemies,target_index,power,mod = 1) => {
    ...
    let damage = calculate_damage(self.data.level,power,self.data.attack,target.data.defence,mod);
...
const bear = {
    attack_obj: function(allies,enemies,self) {
        ...
        return () => {
            if (!resting--)
            {
                if (can_heal && data.health < min_heal_health)
                {
